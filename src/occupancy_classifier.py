...
class OccupancyClassifier:
    """
    Classify parking spaces as occupied or empty
    Uses SVM or Random Forest with extracted features
    """
    ...
    def evaluate(self, X_test: np.ndarray, y_test: np.ndarray):
        ...
        try:
            # We need the probabilities for the 'Occupied' class
            ...
            plt.plot([0, 1], [0, 1], color='gray', lw=2, linestyle='--') # 50/50 line
            plt.xlim([0.0, 1.0])
            plt.ylim([0.0, 1.05])
    ...
    def predict(self, features: np.ndarray) -> Tuple[int, float]:
        """
        Predict occupancy status for a single parking space
